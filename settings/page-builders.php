...
class OptionPageBuilder
{
    ...
    public function __construct($page, $scripts = array(), $styles = array())
    {
        ...
        $this->styles[] = array(
            ...
            'src'=> $boatdealer_pcs_settings_config['base_uri'] . 'styles/admin-settings.css', 
            'enqueue' => true);
    ...
    {
        //    	    wp_enqueue_style( 'bill-admin-styles' , BOATDEALERURL.'settings/styles/admin-settings.css');
        ...
        foreach ($this->scripts as $script) {
            ...
            if (isset($script['enqueue']) && $script['enqueue']) {
                if (isset($script['src']) && !wp_script_is($script['handle'], 'registered')) {
                    wp_register_script($script['handle'], $script['src'], $deps,BOATDEALERPLUGINVERSION);
                }
                ...
                if (isset($script['src']) && !wp_script_is($script['handle'], 'registered')) {
                    wp_register_script($script['handle'], $script['src'], $script['deps'],BOATDEALERPLUGINVERSION);
                }
            }
        }
        // Process the Styles
        foreach ($this->styles as $style) {
            ...
            if (isset($style['enqueue']) && $style['enqueue']) {
                if (isset($style['src']) && !wp_style_is($style['handle'], 'registered')) {
                    wp_register_style($style['handle'], $style['src'], $deps,BOATDEALERPLUGINVERSION);
                }
                ...
                if (isset($style['src']) && !wp_style_is($style['handle'], 'registered')) {
                    wp_register_style($style['handle'], $style['src'], $style['deps'],BOATDEALERPLUGINVERSION);
                }
            }
        }
        // 	} Endif
    ...
    public function sanitize_textarea($value, $old_value) {
        ...
        if (isset($value['boatdealer_fieldfeatures'])) {
            $value['boatdealer_fieldfeatures'] = wp_kses_post($value['boatdealer_fieldfeatures']);
        }
