...
function boatdealer_plugin_health()
{
    ...
    echo '<br />';
    echo '</strong>';
    $memory_result = ob_get_contents();
    ob_end_clean();
    function boatdealer_plugin_add_memory_test($tests)
    {
        $tests['direct']['memory_plugin'] = array(
            'label' => __('My Memory Test','boatdealer'),
            'test' => 'boatdealer_plugin_memory_test',
        );
        return $tests;
    }
    $perc = $memory['usage'] / $memory['wp_limit'];
    if ($perc > .7) {
        add_filter('site_status_tests', 'boatdealer_plugin_add_memory_test');
    }
    function boatdealer_plugin_memory_test()
    {
