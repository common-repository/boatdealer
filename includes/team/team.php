...
function boatdealer_show_team()
{
    $boatdealerTeam = boatdealer_reorder_terms();
    $qagents = count($boatdealerTeam);
    $output = '';
    $output .= '<div class="container boatdealer_wrapteam">';
    if ($qagents > 0) {
         $i = 0;
        foreach ($boatdealerTeam as $term) {
          if($i % 3 == 0 or $i == 0)  
           ...
           // $image = aq_resize( $image, 800, 400, true ); //resize & crop the image
            ...
            // 6 divide 50%
            // 4  33%
            $output .= '<div class="col-md-4 col-lg-4 boatdealer_wrapindividual">'; // start single member
            $output .= '<div class="reatestateimg-team">';
            $output .= '<img class="reatestateimg-img_team" src="' . $image . '" />';
            $output .= '</div>'; //image
            $output .= '<div class="boatdealer_nameteam">';
            $output .= $name;
            $output .= '</div>'; //title
            $output .= '<div class="boatdealer_team_function">';
            $output .= $function;
            $output .= '</div>'; //function
            $output .= '<div class = "boatdealer_descriptionteam">';
            $output .= substr($description, 0, 140);
            $output .= '</div>';
            if (!empty($phone)) {
                $output .= '<div class = "boatdealer_phone_team">';
                $output .= '<i class="fa fa-phone" aria-hidden="true"></i>';
                $output .= '&nbsp;' . $phone;
                $output .= '</div>';
            }
            if (!empty($skype)) {
                $output .= '<div class = "boatdealer_skype_team">';
                $output .= '<i class="fa fa-skype" aria-hidden="true"></i>';
                $output .= '&nbsp;' . $skype;
                $output .= '</div>';
            }
            if (!empty($email)) {
                $output .= '<div class = "boatdealer_email_team">';
                $output .= '<i class="fa fa-envelope-o" aria-hidden="true"></i>';
                $output .= '&nbsp;' . $email;
                $output .= '</div>';
            }
            $output .= '<div class = "boatdealer_iconswrap">';
            if (!empty($facebook)) {
                $output .= '<div class = "boatdealer_iconsteam">';
        ...
        $output .= __("No Team Member Added!", "boatdealer") ;
    }
    $output .= '</div>'; //container
    return $output;
}
add_shortcode('boatdealer_team', 'boatdealer_show_team'); ?>