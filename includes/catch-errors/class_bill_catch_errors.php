...
namespace boatdealer_BillCatchErrors;
// created 06/23/23
...
function boatdealer_bill_hooking_catch_errors()
{
        if (function_exists('is_admin') && function_exists('current_user_can')) {
            if(is_admin() and current_user_can("manage_options")){
                ...
                $boatdealer_plugin_slug = 'restore-classic-widgets';
...
add_action("wp_ajax_bill_minozzi_js_error_catched", "boatdealer_BillCatchErrors\\bill_minozzi_js_error_catched");
add_action("wp_ajax_nopriv_bill_minozzi_js_error_catched", "boatdealer_BillCatchErrors\\bill_minozzi_js_error_catched");
    ...
    if (isset($_REQUEST)) {
        ...
        foreach ($errors as $error) {
            ...
            if ($log_error) {
                ...
                if ($boatdealer_plugin_slug  == 'wptools')
...
class boatdealer_bill_catch_errors
{
...
new boatdealer_bill_catch_errors();
//
